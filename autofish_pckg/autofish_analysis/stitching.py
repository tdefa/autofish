...
## todo document and clean stich_dico_spots
...
def stich_segmask(dict_stitch_img, # np.load(f"/media/tom/T7/Stitch/acquisition/2mai_dict_stitch.npy",allow_pickle=True).item()
    ...
    for image_name in dict_stitch_img:
        ...
        max_z = int(np.max(np.array(list(dict_stitch.values()))[:,0]))+1
        final_shape_xy = np.array([ image_ly * nb_tiles_y + margin, image_lx * nb_tiles_x + margin]) #ten pixels margin
        ...
        for path_ind_mask in tqdm(list(Path(path_mask).glob("*.tif"))[:]):
            ...
            if image_position not in dict_stitch:
                continue
            print(path_ind_mask.name)
            ind_mask = tifffile.imread(path_ind_mask)
            image_position = "pos" + path_ind_mask.name.split("pos")[1].split(".")[0].split("_")[0]
            z_or, y_or, x_or = dict_stitch[image_position]
            ...
            ###
            if compute_dict_centroid:
                compute_dict_centroid_ind_mask = compute_dict_centroid(mask_nuclei = ind_mask,
                                                                       dict_simu=None,
                                                                       offset=np.array([0, y_or, x_or]))
                for key in compute_dict_centroid_ind_mask.keys():
                    dict_centroid[key] = compute_dict_centroid_ind_mask[key]
                print(f'len dict_centroid {len(dict_centroid)}')
                print()
                (Path(path_to_save_mask) / "dict_centroid").mkdir(parents=True, exist_ok=True)
                np.save((Path(path_to_save_mask) / "dict_centroid") / image_name, dict_centroid)
    ...
    if compute_dict_centroid:
        ...
        return  dict_centroid, dict_df_matching_new_cell_label
    else:
        return dict_df_matching_new_cell_label

def stich_from_dico_img(
                dico_stitch, # np.load(f"/media/tom/T7/Stitch/acquisition/2mai_dico_stitch.npy",allow_pickle=True).item()
